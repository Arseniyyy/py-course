...
print('Привет ', name)
```
Запустите этот код и вы увидите сообщение с вашим именем. Функция может принимать неограниченное число аргументов, поэтому мы передаём переменную `name` в функцию `print()`.
...
Предположим, у нас есть переменная `a`, которой присвоенно число 10:

```python
a = 10
```
Что, если мы захотим изменить значение `a` и прибавить 2? Мы можем это сделать либо стандартным способом, просто написав:
...
a = 10
a //= 3  # деление нацело, получается 3

a = 10
a **= 3  # Возведение в степень, получается 1000
```
...
## Тип данных `float`
Вещественные числа представляют собой числа с плавающей точкой, то есть числа, которые могут иметь дробную часть. Это позволяет нам работать с числами, которые не являются целыми. Давайте рассмотрим основные аспекты работы с типом данных `float` в Python.
...
Входные данные: price = 100, discount = 20

Ожидаемый вывод: final_price = 80

### Задача 4: Расчет площади треугольника
Напишите программу, которая принимает длины сторон треугольника от пользователя и вычисляет его площадь с помощью формулы Герона, используя присваивание переменных.

Входные данные: a = 5, b = 6, c = 7

Ожидаемый вывод: area = 14.696938456699069
...
### Задача 10: Обмен значениями двух переменных
...
### Задача 14: Конфеты в день рождения
