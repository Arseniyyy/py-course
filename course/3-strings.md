...
Почему же возникает эта ошибка при попытке запустить код выше? Всё просто. Python нам говорит, что нет такого индекса в строке. А есть индексы 0, 1 и 2, так как строка состоит из трёх символов. Другими словами, мы пытаемся получить доступ к символу, которого в строке не существует.
...
### Сколько символов?
Напишите программу, которая принимает входные данные в виде строки и выводит количество символов в строке. Подсказка: для этого надо использовать функцию `len()`, информацию о которой ты можешь найти на данном сайте https://realpython.com/len-python-function/#:~:text=Remove%20ads-,Using%20len()%20With%20Built%2Din%20Sequences,-A%20sequence%20is.
...
### Кто крадёт символы?
Напишите программу, которая принимает входные данные в виде строки, назначает эту строку переменной text, и выводит первые три символа, затем последние три символа, а затем средний символ (строка всегда имеет нечетную длину).
...
### Сбой
В таинственном космическом агентстве произошла небольшая сбойка в программе управления роботами-исследователями. На каждом роботе записаны два числа, но их значения стерлись. Команде требуется твои навыки, чтобы восстановить эти числа и вычислить их сумму!
...
#### Формат ввода:
Два числа, разделённых пробелом на двух разных строках.
